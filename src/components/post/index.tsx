...
const Post = ({ author, content, published }: PostProps) => {
  ...
  const handleValidComment = (event: FormEvent) => {
    ...
    <article className="w-[832px] mb-6 bg-[#202024] p-12 rounded-lg">
      <header className="w-full flex justify-between">
        <div className="flex gap-6 w-[300px]">
          <Avatar hasBorder imageUrl={author.avatarUrl} />

          <div>
            <h1 className="text-[#E1E1E6] font-bold text-base">
              {author.name}
            </h1>
      ...
      >
        ...
        <footer>
          <button
            ...
            className="bg-[#00B37E] py-4 px-6 rounded-lg text-white font-bold disabled:opacity-[0.7] disabled:cursor-not-allowed"
          >
            Publicar
          </button>
        </footer>
